...
chrome.storage.sync.get("saved", (obj) => 
{
  if(obj["saved"]==undefined)
  {
    obj["saved"]=[]
  }
  ...
  tabWipeButton.addEventListener("click", () => {
    chrome.tabs.query( {windowId: currentWindow}, (tabs) => 
    {
      ...
      for(let i=0; i<tabs.length; i++){
        ...
        if(obj["saved"].indexOf(tabDomain) == -1){
  ...
  unsavedButton.addEventListener("click", async () => {
        onUnsaved = true
        listDiv.innerHTML=""
        titleSpan.innerText="Unsaved Sites"
        chrome.tabs.query( {windowId: currentWindow}, (tabs) => 
        {
          for(let i=0; i<tabs.length; i++){
            let tabDomain = (new URL(tabs[i].url));
            tabDomain = tabDomain.hostname.replace("www.", "");
            if(!obj["saved"].includes(tabDomain))
            {
              ...
              addInput.setAttribute("id", i);
              document.getElementById(i).addEventListener("change", (event) => 
              {
                let deleteDiv = document.getElementById(i).parentNode
                deleteDiv.remove()
                obj["saved"].push(tabDomain)
                chrome.storage.sync.set({"saved": obj["saved"]})
              })
              addP.innerText=tabDomain
            }
          }
        }
      )
  ...
  savedButton.addEventListener("click", async () => {
    onUnsaved = false
    listDiv.innerHTML=""
    titleSpan.innerText="Saved Sites"
    chrome.tabs.query( {windowId: currentWindow}, (tabs) => 
    {
        for(let i=0; i<tabs.length; i++){
          let tabDomain = (new URL(tabs[i].url));
          ...
          if(obj["saved"].includes(tabDomain))
          {
            ...
            document.getElementById(i).addEventListener("change", (event) => 
            {
              let deleteDiv = document.getElementById(i).parentNode
              deleteDiv.remove()
              obj["saved"].splice(obj["saved"].indexOf(tabs[i].url), 1)
              chrome.storage.sync.set({"saved": obj["saved"]})
            })
            let tabDomain = (new URL(tabs[i].url));
            tabDomain = tabDomain.hostname.replace("www.", "");
...
//     chrome.storage.sync.get(key, (obj) => alert(obj["saved"]))
//   }
// });
